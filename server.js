...
bot.onText(/\/start/, (msg) => {
    ...
    // Создаем клавиатуру с кнопкой "Скопировать Chat ID"
    const keyboard = {
        inline_keyboard: [[
            {
    ...
    bot.sendMessage(chatId, 
        `👋 Привет${username ? `, ${username}` : ''}! Я бот FanFik для двухфакторной аутентификации.\n\n` +
        `Ваш Chat ID: \`${chatId}\`\n\n` +
        `📋 *Как использовать:*\n` +
        `1. Нажмите на Chat ID выше, чтобы скопировать его\n` +
        `2. Или используйте кнопку ниже для копирования\n` +
        `3. На сайте FanFik вставьте его в поле привязки Telegram\n` +
        `4. При входе на сайт вы будете получать коды подтверждения здесь\n\n` +
        `🔒 *Безопасность:*\n` +
